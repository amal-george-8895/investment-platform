...
<div class="container-fluid bgclass">
    <div class="row">
        ...
        <div class="col-lg-6" style="display: flex;justify-content: flex-end; align-items: center;">
            <h4 style="color: #ffffff;margin: 12px 30px;font-size: 18px;font-weight: 500;"></h4>
...
<div class="container-fluid my-3">
    <div class="row">
        ...
        <div class="col-lg-6 grid1_2 my-5">
            ...
            <?php
            include 'db_conn.php';
            $userId = $_GET['userId']; // Get user ID from GET parameter


            $sql = "SELECT * FROM `user` WHERE id='$userId'";// Retrieve user information from database
...
//                        $description = $row['description'];
                        ...
                        if ($_SESSION['role'] == 'admin' && $row['role'] == 'client'){
                ...
                <div class=\"row\">
                 ...
                 <div class=\"d-flex flex-column\">
                    <h4>Assign RM</h4>
                    <select id='assignedRm' name=\"assignedRm\" ";
                            $query = "SELECT * FROM `user` where role='rm'"; // Query the database to retrieve all users with role 'rm'
                            $result = $con->query($query);
                            print_r($result);// Print the results of the query
                            if($result ) {
                                while ($row1 = $result->fetch_assoc()) {
                                if ( $assigned_rm != '' && $assigned_rm == $row1['id']){
                                    echo "<option selected  value=".$row1['id']." >". $row1['fname'] . ' ' . $row1['lname'] ."</option>";
                                }else{
